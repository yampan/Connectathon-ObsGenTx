...
# j_map: {'var_name' : (81:'Dose') }  変数名: (NO, '項目名')

j_map = {"id":(112,'ID'), 'kannri_id':(1,'院内管理コード') , 'plan':(114, 'PLAN'), 
    ...
    'CPR1': ( 17, 'CPR1'), 'T1': ( 18, 'T1'), 'N1': ( 19, 'N1'), 'M1': ( 20, 'M1'),
    'S1': ( 21, 'Stage1'), 'MGRNO': (113, '管理名'), 'PLAN': (114, '計画名')}

# log sheet header
log_header = ["ROW", "COLUMN", "Original value", "New value", "Item name",
              "Datetime", 'PID', 'UID' ]
log_width =  [6, 6, 15, 15, 15, 25, 20, 25]


def getUID(hosp):
    """
    各病院のデータがUniqueになる、項目番号を返す。
    Args:
        hosp: 病院コード、ARC, SCH, SMC, SRC
    return:
        UID: [col1, col2, col3, ...]
    """
    if hosp=="ARC":
    ...
    # ( 51, '外部照射部位'), ( 43, '外部照射開始日')
    # ( 45, '外部照射総線量') ( 11, '原発部位')
        return [1, 51, 43, 45, 11]     
    elif hosp=="SRC":
    # ( 51, '外部照射部位'), (116, '放射線治療番号_外部照射') @SRC
        return [1, 51, 116] # Unique-IDとなる、カラム番号（LIST)
    return None
...
def xlcolor(ws, row:int, col:int, color="ACEBF0"):
    ...
    Args:
        ...
        color: 設定する色（RGB: 16進数、F0F0F0)F0F0F0)
    return:
        color: 色
    """
...
def trans2(ws, row, d:dict, ws_log, deb=0):
    """
    Transaction2
    d: dict で示される、項目名(str):値 を sheet: ws に書込し、sheet: ws_log に記録する.
    """
    # d = {"name": value} => col, val
    name = None
    
    if len(d) != 1:
        print(f"trans2: #27 d: dict ERROR: {d}")
        return
    # ---
    for i in d.keys():
        val = d[i]
...
def trans(ws, row, col, val, ws_log, disp="null", pid="null", UID=None, deb=0):
    """
    Transaction
    ワークシートwsの(row, col)の値に(val)を代入して、Sheet: ws_logに記録する。（append)
    Args:
        ws: ターゲット worksheet
        row, col : 対象とするCELL
        val: 書き込む値
        ...
        pid: 値を書き込む、program-ID
        UID(LIST): レコードのユニークIDのカラム番号のリスト
    return:
        log: ws_logに書き込んだ値。
    """
    cell = ws.cell(row=row, column=col)
    if deb: print(f"trans: #56 cell = {cell}")
    old_v = cell.value
    cell.value = val
    if UID is None:
        uid = "null"
        ...
        for i in UID:
            ...
            values.append(v)
        uid = '|'.join(values)
        if deb: print(f"trans: uid = {uid}")
    # --- trans
    log_dat = [row, col, old_v, val, disp, JST(), pid, uid]
...
def search(ws, col_no, val, n, deb=0):
    """
...
def search2(ws, col_no, val, n, deb=0):
    """
    ワークシート内で指定された値を検索する関数(row, col は1 始まり)

    Args:
        ws (openpyxl.worksheet.worksheet.Worksheet): ワークシートオブジェクト
        col_no (int): 検索する列番号(1 始まり)
        val (str): 検索する値
        n (int): 検索を開始する行番号(1 始まり)

    Returns:
        tuple: 見つかった場合は列データのリストと行番号、見つからなかった場合はNone, None
        列データは、col[1] 始まり。col[0] = row_no
    """
    if deb: print(f"#search: #151: {val}")
    for n,row in enumerate(ws.iter_rows(min_row=n), start=n):
        if str(row[col_no - 1].value) == val:
            col = [cell.value for cell in row]
            return [n]+col, row[0].row
    return None, None


def search3(ws, col_no, val, n, deb=0):
    """
    高速化： search2 : search3 = 3.35 : 0.91    3.6 倍速い
    ワークシート内で指定された値を検索する関数(row, col は1 始まり)
    iter_cols() を使用する。
    ...
    Returns:
        tuple: (cols, n) 列データのリストと行番号、見つからなかった場合は None, None
        列データは、col[1] 始まり。col[0] = row_no
    """
    found = 0
    col = [c for c in ws.iter_cols(min_col=col_no, max_col=col_no)]
    col = col[0]
    #print("#392: col=", col[:10])
    for l,j in enumerate(list(col), start=1):
        if l < n:
            continue
        #print("j=", j)
        if j.value == val:
            ...
            break
    # ---
    if deb: print("j=", j.value, ' l=', l)
    if found:
        return getRow(ws, l), l
    else:
        return None, None
...
def openXl(fn, sheet_name=None):
    ...
    # info
    print(f"{line()} {fn}, sheet: {sheet_name}, max row={sheet.max_row}, col={sheet.max_column}")
    ...
    for y in range(1, sheet.max_column+1):
        ...
        #print(f"{line()} title[{y}] = {title[y]}")
    print(f"{line()} title-1:{title[1]}, title-max:{title[sheet.max_column]}")
...
if __name__ == "__main__":
    ...
    # "mylib\\aaa.xlsx", を output_file="mylib\\bbb.xlsx"　にコピーする。
    wb, ws, ws2 = copy_worksheet_with_styles()
    ws2 = sort2(wb, ws2, key=(1, 3)) # OK
...
# Main 4
if __name__ == "__main1__":
    print("このスクリプトは直接実行されました", line())
    
    print("location=", location())
    print("line=", line())
    #fn = 'JRODe_SRC_Sample.xlsx'
    #fn = "JRODe_test.xlsx"
    fn = "JRODe_SCH_Sample.xlsx"
    ...
    for i in range(NNN):
        ...
        if col is not None:
            ...
            pass
    t_en = time.time()
    print(f"time diff = {t_en - t_st:.2f}")
    print("search2: n=", n)
    
    # search3  TEST
    t_st = time.time()
    for i in range(NNN):
        col, n = search3(ws, 1, val, 2)
        if col is not None:
            #print(f"#558 found: {col[:10]}, {n}")
            pass
        else:
    ...
    # --- trans
    if 1:
        # trans(ws, row, col, val, ws_log, disp="null", pid="null", UID=None, deb=0)
        trans(ws2, 3, 14, "test-dat", ws3, "test", "test-prog", [1,113,114])
